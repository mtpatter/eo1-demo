...
class test(object):
    ...
    def createMetadata(self):
        ...
        filename = self.filePre+"_MTL_L1T.TIF"
    ...
    def writeTif(self,filename):

        driver = gdal.GetDriverByName("GTiff")
        driver.Register()
        src = gdal.Open(self.filePre+"_B02_L1T.TIF")
        ...
        dest = driver.Create(filename,cols,rows,numBands,datatype)
...
if __name__ == '__main__':
    ...
    path = '/glusterfs/osdc_public_data/eo1/ali_l1g/'+str(year)+'/'+str(day)+'/'+str(name)+'_ALI_L1G/'+str(name)

    f = test(path)
    f.setUpTest()
    f.setUpTrain('FourClassTrainingSet.txt')

    f.addBand(np.arange(9))
    f.addRatio((3,7))
    f.addRatio((4,8))

              
    print "About to begin Test"
    f.svmTrain()
    f.svmTest()
